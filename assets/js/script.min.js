...
(function ($) {
  $(document).ready(function () {
    ...
    const primaryMenuBreakpoint = 980;
    const $primaryMenu = $('#primary-menu');
    const $primaryMenuItems = $primaryMenu.find('li.menu-item-has-children');
    const $desktopSecondLevelItems = $primaryMenu.find('> li > ul.sub-menu > li.menu-item-has-children');

    const closeSubTree = ($items) => {
      $items.each(function () {
        $(this)
    ...
    const teardownPrimaryMenuAccordion = () => {
      $primaryMenu.off('click.mobileMenuAccordion', 'li.menu-item-has-children > a');
      closeSubTree($primaryMenuItems);
    };

    const closeDesktopSubmenus = () => {
      $desktopSecondLevelItems.removeClass('submenu-open');
      $desktopSecondLevelItems.find('.sub-menu').slideUp(200);
    };

    const setupPrimaryMenuAccordion = () => {
      // Обработчик для всех пунктов меню с submenu (используем делегирование для всех уровней)
      $primaryMenu.off('click.mobileMenuAccordion', 'li.menu-item-has-children > a')
                  .on('click.mobileMenuAccordion', 'li.menu-item-has-children > a', function (e) {
        const $parentItem = $(this).parent();
        const $submenu = $parentItem.children('.sub-menu');

        // Если у пункта нет submenu, разрешаем обычную навигацию
        if (!$submenu.length) {
          return;
        }
        ...
        const isFirstLevel = $parentItem.parent().is('#primary-menu');

        // Если это пункт первого уровня, закрываем все другие открытые меню первого уровня
        if (isFirstLevel) {
          closeSubTree($primaryMenuItems.not($parentItem));
        }

        // Переключаем текущее меню: если открыто - закрываем, если закрыто - открываем
        if (isOpen) {
    ...
    const teardownDesktopSubmenuAccordion = () => {
      $desktopSecondLevelItems.find('> a').off('click.desktopPrimarySubmenu');
      closeDesktopSubmenus();
    };

    const setupDesktopSubmenuAccordion = () => {
      $desktopSecondLevelItems.find('> a').off('click.desktopPrimarySubmenu').on('click.desktopPrimarySubmenu', function (e) {
        if (window.innerWidth <= primaryMenuBreakpoint) {
          return;
        }
    ...
    const handlePrimaryMenuAccordion = () => {
      if (!$primaryMenu.length) {
        return;
      }

      if (window.innerWidth <= primaryMenuBreakpoint) {
        setupPrimaryMenuAccordion();
        teardownDesktopSubmenuAccordion();
      } else {
        teardownPrimaryMenuAccordion();
        setupDesktopSubmenuAccordion();
      }
    };

    handlePrimaryMenuAccordion();
    $(window).on('resize.mobilePrimaryMenu', handlePrimaryMenuAccordion);
    ...
    // Close popup (login and others)
    $('.close-contactpage, .page-overlay').on('click', function () {
      $('.popup-content-contactpage, .page-overlay').fadeOut();
      // Также скрываем родительский контейнер формы логина и сбрасываем класс
      $('#login_form_popup').fadeOut().removeClass('show-login-popup-active');
    })
    ...
    if ($('#shipping_form').length) {
      $('#shipping_form').on('submit', function (e) {
        ...
        if ($('.woocommerce-checkout').length) {
          ...
          $('#shipping_first_name').val(formDataArray.shipping_first_name)
          $('#shipping_last_name').val(formDataArray.shipping_last_name)
          $('#shipping_company').val(formDataArray.shipping_company)
          $('#shipping_address_1').val(formDataArray.shipping_street)
          $('#shipping_address_2').val('')
          $('#shipping_postcode').val(formDataArray.shipping_postal)
          $('#shipping_city').val(formDataArray.shipping_city)
          $('#shipping_state').val('IL')
          $('#shipping_email').val(formDataArray.shipping_email)
          $('#shipping_address_type').val(formDataArray.shipping_address_type)
          $('#shipping_house_number').val(formDataArray.shipping_house_number)
          $('#shipping_apartment_number').val(formDataArray.shipping_apartment)
          $('#shipping_phone').val(formDataArray.shipping_phone)
          ...
          success: function (response) {
            ...
            if (response.success) {
              if (response.multiAccount) {
                ...
                let template = '<div class="multi-account">';
                template += '<p>בחר את החשבון שלך</p>';
                $(accounts).each((i, item) => {
                  template += '<label>';
                  template += `<input type='radio' class='user_multi_account' name='user_account' value='${item.CUSTNAME}' ${i===0 ? 'checked' : ''}>`;
                  template += '<span class="f-radio"></span>'
                  template += `<span>${item.FGEF_FIRSTNAME} ${item.FGEF_LASTNAME}</span>`;
                  template += '</label>';
                })

                template += '</div>'

                $('#login_by_phone .contact-form-help-row').append(template);
    ...
    if ($('#resend_sms').length) {
      $('#resend_sms').on('click', function (e) {
        ...
        $.ajax({
          ...
          complete: function () {
            $(form).find('button[type="submit"]').prop('disabled', false)
            $('#resend_sms').prop('disabled', false)
          },
        })
      })
    }
    ...
    $('#checkout_payment').on('click', function (e) {
      ...
      $.ajax({
        ...
        url: window.geffen.ajax_url,
        data,
        timeout: 30000, // 30 seconds timeout to prevent 499 errors
        beforeSend: function () {
          // Show Preloader
          $('.checkout-ring').fadeIn()
          $(btn).prop('disabled', true)
        ...
        success: function (response) {
          ...
          if (response && response.success) {
            ...
            } else if (response.data && response.data.thankyoulink) {
              window.location.href = response.data.thankyoulink;
            } else {
              console.error('Invalid response structure:', response)
              alert('שגיאה: תגובה לא תקינה מהשרת')
              $('.checkout-ring').fadeOut()
              $(btn).prop('disabled', false)
            }
          ...
          } else if (response && response.thankyoulink) {
            // Legacy format support
