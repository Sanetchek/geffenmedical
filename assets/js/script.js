...
(function ($) {
  $(document).ready(function () {
    // Force-hide login popup on load
    (function hideLoginOnLoad() {
      const parentPopup = $('#login_form_popup');
      const popup = $('#login_form_popup .popup-content-contactpage');
      const overlay = $('.page-overlay');
      parentPopup.removeClass('show-login-popup-active')
        .hide().css({ display: 'none', visibility: 'hidden', opacity: 0 });
      popup.hide().css({ display: 'none', visibility: 'hidden', opacity: 0 });
      overlay.hide();
    })();
  ...
  /* Функция для показа попапа входа */
  function showLoginPopup() {
    const popup = $('#login_form_popup .popup-content-contactpage.logip-page-formcontent.registratet-popup-forms');
    const parentPopup = $('#login_form_popup');
    
    if (popup.length && parentPopup.length) {
      // Переносим контейнер попапа в <body>, чтобы исключить влияние stacking context хедера
      if (!parentPopup.data('moved-to-body')) {
        parentPopup.appendTo('body');
        parentPopup.data('moved-to-body', true);
      }
  
      // Показываем родительский контейнер
      parentPopup.addClass('show-login-popup-active');
      parentPopup.show();
      parentPopup[0].style.setProperty('display', 'block', 'important');
      parentPopup[0].style.setProperty('visibility', 'visible', 'important');
      parentPopup[0].style.setProperty('opacity', '1', 'important');
      
      // Показываем сам попап
      popup.addClass('show-login-popup-active');
      popup[0].style.setProperty('display', 'block', 'important');
      popup[0].style.setProperty('visibility', 'visible', 'important');
      popup[0].style.setProperty('opacity', '1', 'important');
      popup[0].style.setProperty('position', 'fixed', 'important');
      popup[0].style.setProperty('z-index', '99999999', 'important');
      popup[0].style.setProperty('top', '50%', 'important');
      popup[0].style.setProperty('left', '50%', 'important');
      popup[0].style.setProperty('transform', 'translate(-50%, -50%)', 'important');
      
      // Также используем fadeIn для анимации
      popup.fadeIn(300);
      $('.page-overlay').fadeIn();
    } else {
      // Fallback: используем стандартную функцию
      const popupFallback = $('#login_form_popup .popup-content-contactpage');
      if (popupFallback.length) {
        show_popup(popupFallback);
      }
    }
  }
  ...
  /* Обработка клика на user-site-login для установки display: block */
  $(document).on('click', '.user-site-login .show-login-popup', function (e) {
    e.preventDefault();
    showLoginPopup();
  });
    ...
    // Close popup (login and others) — delegated to work after DOM moves
    $(document).on('click', '.close-contactpage, .page-overlay', function (e) {
      e.preventDefault();
      e.stopPropagation();
      $('.popup-content-contactpage, .page-overlay').fadeOut();
      $('#login_form_popup')
        .fadeOut()
        .removeClass('show-login-popup-active')
