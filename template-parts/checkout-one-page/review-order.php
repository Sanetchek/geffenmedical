...
<section class="shop_table shop_table_responsive">
	<div id="customer_billing_details">
		<?php get_template_part('template-parts/checkout-one-page/checkout-fields') ?>
	</div>
	...
	<?php foreach (WC()->cart->get_coupons() as $code => $coupon): ?>
		<div class="cart__grid cart-discount coupon-<?php echo esc_attr(sanitize_title($code)); ?>">
			<div class="cart__grid_head"><?= __('קוד קופון ', 'geffen') ?> 	<?php echo esc_attr(sanitize_title($code)); ?></div>
			<div class="cart__grid_content" data-title="<?php echo esc_attr(wc_cart_totals_coupon_label($coupon, false)); ?>">
				<!-- Change Remove BTN class -->
				<?php wc_cart_totals_coupon_html($coupon); ?>

				<button type="button" class="geffen-remove-coupon" data-coupon="<?php echo esc_attr($code); ?>">
					<?= __('[הסר קופון]', 'geffen') ?>
				</button>
			</div>
		</div>
