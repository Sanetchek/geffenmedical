<div class="checkout-next">
  ...
  if ($user_id) :
    ...
    $is_policy = get_user_meta($user_id, 'geffen_privacy_policy', true);

    $user_info = get_userdata($user_id);
    $is_user_email = $user_info ? $user_info->user_email : '';

    if ($user_info && $is_policy && $is_user_email) :
      ...
      $payment = (!$subscribed && $found_product) ? '' : 'id="checkout_payment"';
      $payment_load = (!$subscribed && $found_product) ? '' : '<div class="checkout-ring"><div></div><div></div><div></div><div></div></div>';
      ?>

      <button <?= $payment ?> class="<?= $popup_subscribe ?> checkout-payment cart__proceed_btn checkout-button alt wc-forward<?php echo esc_attr(wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : ''); ?>">
        <?php _e('המשך לתשלום ', 'geffen'); ?>
        <?= $payment_load ?>
      </button>

      <?php if (!$subscribed && $found_product) get_template_part('template-parts/checkout-one-page/subscribe', 'popup'); ?>
    <?php else: ?>
