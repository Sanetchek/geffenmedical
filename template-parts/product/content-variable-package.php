...
<div class="product-info-tabs-row">
  <?php
    ...
    $is_case = in_array('case', $tab);
    $is_singular = in_array('singular', $tab);
    $is_tab = $is_case && $is_singular;
  ?>
  ...
  <?php
    $active_single = !$is_tab ? 'active' : '';

    $product_id = absint($product->get_id());
    $permalink = $product->get_permalink();
    $filtered_permalink = apply_filters('woocommerce_add_to_cart_form_action', $permalink);
    $args = ['product' => $product];
  ?>

  <?php if ($is_case): ?>
    <form class="variations_form cart"
      action="<?php echo esc_url( $filtered_permalink ); ?>"
      ...
      <div class="product-info-tab-content package-product calculate-price active" id="case-content">
        <div class="tastes-content">
          <?php get_template_part('template-parts/product/package/case', '', $args) ?>
        </div>
      </div>
    </form>
  <?php endif ?>
  ...
  <?php if ($is_singular) : ?>
    ...
    method="post" enctype='multipart/form-data'
    data-product_id="<?php echo $product_id; ?>">
      <div class="product-info-tab-content singularity-products calculate-price <?= $active_single ?>" id="singular-content">
        <?php get_template_part('template-parts/product/package/singular', 'package', $args) ?>
      </div>
    </form>
  <?php endif ?>
</div>